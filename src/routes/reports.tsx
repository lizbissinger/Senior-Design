...
export async function GetLoadCount(driver?: string, dateRange?: any) {
  try {
    const from = dateRange?.from ? dateRange.from : null;
    const to = dateRange?.to ? dateRange.to : null;
    const _driver = (driver != null && driver?.length > 0) ? driver : null;

    const response = await fetch(`${api}/reports/loadCount?driver=${_driver}&from=${from}&to=${to}`, {
      method: "GET"
    });
...
export async function GetExpenses(dateRange?: any) {
  try {
    const from = dateRange?.from ? dateRange.from : null;
    const to = dateRange?.to ? dateRange.to : null;

    const response = await fetch(`${api}/reports/expenses?from=${from}&to=${to}`, {
      method: "GET"
    });
